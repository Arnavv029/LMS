...
const Navbar = () => {
    ...
    const { user } = useUser() 
    const {navigator , isEducator} = useContext(AppContext)
    ...
    return (
     ...
     lg:px-36 border-b border-gray-500 py-4 ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
            ...
            <img onClick={ () => navigator('/')} src={assets.logo} className='w-28 lg:w-32 cursor-pointer' />
            <div className='hidden md:flex items-center gap-5 text-gray-500'>
                <div className='flex items-center gap-5'>
                    {user &&
                        <> 
                        <button onClick={ () => navigator('/educator')}>{ isEducator ? 'Educator Dashboard' : 'Become Educator'}</button>
                        | <Link to='/myenrollment'>My Enrollments</Link>
                        </> 
                    }
                </div>
                {user ? <UserButton /> :
