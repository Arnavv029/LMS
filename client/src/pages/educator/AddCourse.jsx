...
const AddCourse = () => {
  ...
  const[currentlyChapterId, setCurrentlyChapterId] = useState(null)
  ...
  const handleLecture = (action, chapterId, lectureIndex ) => {
    ...
    if(action === 'add'){
      setCurrentlyChapterId(chapterId)
      setShowPopup(true)
    }else if(action === 'remove'){
  ...
  const addLecture = () => {
    setChapters(
      chapters.map((chapter) => {
        if(chapter.chapterId === currentlyChapterId) {
          const newLecture = {
            ...lectureDetails,
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={ (e) => e.preventDefault() }  className='flex flex-col gap-4 max-w-md w-full text-gray-500'> 
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'> 
            ...
            placeholder='0' className='outline-none md:py-2.5 py-2 w-28 rounded border border-gray-500' required />
          </div>

          <div className='flex md:flex-row flex-col items-center gap-3'> 
            <p>Course Tumbnail</p> 
            <label htmlFor="thumbnailImage" className='p-3 bg-blue-500 rounded'>
        ...
        <div> 
          ...
          placeholder='0' max={100} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required />
        </div>
            ...
            <div key={index} className='bg-white border rounded-lg mb-4'> 
              <div className='flex justify-between items-center p-4 border-b'> 
                <div className='flex items-center'>
                  ...
                  src={assets.down_arrow_icon} width={14} alt="" className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} />
          ...
          { showPopup && ( 
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'> 
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80' >
                ...
                <button type='button' onClick={() => addLecture()} className='bg-black text-white w-max py-2.5 px-8 rounded my-4'>
                  ADD 
                </button>
